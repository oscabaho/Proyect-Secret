...
namespace ProyectSecret.Inventory.Items
{
    ...
    public class WeaponItem : MysteryItem, IUsableItem, IEquipable
    {
        ...
        {
            // Implementa aquí la lógica de uso del arma (por ejemplo, equipar o atacar)
            Debug.Log($"Usando arma: {DisplayName}");
        }
        ...
        /// </summary>
        /// <summary>
        /// Evento público para notificar que el arma infligió daño (útil para logros, UI, etc).
        /// </summary>
        public event System.Action<GameObject, GameObject, int> OnWeaponDamage;
        ...
        public virtual void ApplyDamage(GameObject owner, GameObject target)
        {
            ...
            if (damageable != null)
            {
                ...
                OnWeaponDamage?.Invoke(owner, target, weaponDamage);
            }
        }
